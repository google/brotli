...
/* GENERATED CODE BEGIN */
  ...
  function initState(s, input) {
    ...
    s.blockTrees[0] = 7;
    s.distRbIdx = 3;
    let /** @type {number} */ maxDistanceAlphabetLimit = calculateDistanceAlphabetLimit(0x7FFFFFFC, 3, 120);
  ...
  function readHuffmanCodeLengths(codeLengthCodeLengths, numSymbols, codeLengths, s) {
    ...
    let /** @type {!Int32Array} */ table = new Int32Array(33);
  ...
  function readMetablockHuffmanCodesAndContextMaps(s) {
    ...
    for (let /** @type {number} */ i = 0; i < s.numLiteralBlockTypes; ) {
      let /** @type {number} */ limit = Math.min(i + 96, s.numLiteralBlockTypes);
      for (; i < limit; ++i) {
        if (s.bitOffset >= 16) {
          s.accumulator32 = (s.shortBuffer[s.halfOffset++] << 16) | (s.accumulator32 >>> 16);
          s.bitOffset -= 16;
  ...
  function copyUncompressedData(s) {
    ...
    let /** @type {number} */ chunkLength = Math.min(s.ringBufferSize - s.pos, s.metaBlockLength);
  ...
  function writeRingBuffer(s) {
    let /** @type {number} */ toWrite = Math.min(s.outputLength - s.outputUsed, s.ringBufferBytesReady - s.ringBufferBytesWritten);
    if (toWrite !== 0) {
  ...
  function doUseDictionary(s, fence) {
    ...
    if (address < 0) {
      ...
      let /** @type {!ByteBuffer} */ dictionaryData = data;
  ...
  function copyRawBytes(s, data, offset, length) {
    ...
    let /** @type {number} */ copyNibbles = Math.min(halfAvailable(s), length >> 1);
    if (copyNibbles > 0) {
      let /** @type {number} */ readOffset = s.halfOffset << 1;
  ...
  {
    let /** @type {!ByteBuffer} */ dictionaryData = new Int8Array(122784);
