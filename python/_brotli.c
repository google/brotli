...
"  data (bytes): The input data;\n"
"                MUST provide C-contiguous one-dimensional bytes view.\n"
"  output_buffer_limit (int): A best-effort cap on the output-buffer size.\n"
"                The buffer will not be grown further once its allocated size\n"
"                meets or exceeds this value, but the check is applied at the\n"
"                brotli decoder's block granularity: the returned data may\n"
"                exceed the requested limit by up to one decoder block (on the\n"
"                order of tens of kilobytes). If the limit is reached, further\n"
"                calls to process (potentially with empty input) will continue\n"
"                to yield more data. Following 'process()' must only be called\n"
"                with empty input until 'can_accept_more_data()' once returns\n"
"                True.\n"
"\n"
"Returns:\n"
