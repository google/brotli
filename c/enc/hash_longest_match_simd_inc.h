...
static void FN(Prepare)(
  ...
  if (one_shot && input_size <= partial_prepare_threshold) {
    ...
    memset(num, 65535, self->bucket_size_ * sizeof(num[0]));
  }
  memset(self->tags_, 0,
         self->bucket_size_ * self->block_size_ * sizeof(self->tags_[0]));
  memset(self->buckets_, 0,
         self->bucket_size_ * self->block_size_ * sizeof(self->buckets_[0]));
}
...
static BROTLI_INLINE void FN(Store)(
  ...
  const uint32_t hash = FN(HashBytes)(&data[ix & mask], self->hash_shift_);
  const uint32_t key = hash >> TAG_HASH_BITS;
...
static BROTLI_INLINE void FN(FindLongestMatch)(
  ...
  }
  {
    ...
    if (!(num[key] <= 65535 - self->block_size_)) {
      uint64_t mask = (1ULL << (64 - head)) - 1;
      matches &= mask;
    }
