...
<<<<<<< HEAD
  ...
  if (ntrees > 0 && max_table_size > 0 &&
      max_table_size > (SIZE_MAX / sizeof(HuffmanCode)) / ntrees) {
    group->codes  = NULL;
    group->htrees = NULL;
    return BROTLI_FALSE;
  }
  ...
  /* Guard combined allocation overflow (code_size + htree_size). */
  if (htree_size > 0 && code_size > SIZE_MAX - htree_size) {
    group->codes  = NULL;
    group->htrees = NULL;
    return BROTLI_FALSE;
  }
