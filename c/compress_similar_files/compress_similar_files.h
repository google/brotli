...
#ifndef BROTLI_COMPRESS_SIMILAR_H_
#define BROTLI_COMPRESS_SIMILAR_H_
...
void FreeBlockSplits(BlockSplitFromDecoder* block_splits) {
  free(block_splits->types);
  free(block_splits->positions_begin);
  free(block_splits->positions_end);
}

/* Adjusts old backward references for the case when [start, end) range
 * is removed from an original file.
 * backward_references_size should be less then
 *        allocated size for backward_references.
 */
size_t RemoveBackwardReferencesPart(const BackwardReferenceFromDecoder* backward_references,
  ...
  BackwardReferenceFromDecoder* new_backrefs_array =
               (BackwardReferenceFromDecoder*)malloc(
               sizeof(BackwardReferenceFromDecoder) * backward_references_size);
  size_t new_back_ref_size = 0;
  for (int i = 0; i < backward_references_size; ++i) {
    ...
    if (ref.distance > max_distance) {
      if (ref.position < start) {
        new_backrefs_array[new_back_ref_size] = ref;
        new_back_ref_size++;
      } else if (ref.position >= end) {
        new_position = ref.position - (end - start);
        new_max_distance = MIN(new_position, ref.max_distance);
        new_backrefs_array[new_back_ref_size] =
                      (BackwardReferenceFromDecoder){new_position, ref.copy_len,
                                                ref.distance, new_max_distance};
        new_back_ref_size++;
      }
    }
    /* Backward reference */
      ...
      if (ref.position < start) {
        if (ref.position + ref.copy_len < start) {
          ...
          if (start - ref.position >= MIN_COPY_LEN) {
      ...
      /* If it's after removing content then look at
        ...
        if (ref.position - ref.distance >= start && ref.position - ref.distance < end) {
          if (ref.position - ref.distance + ref.copy_len - 1 >= end + MIN_COPY_LEN) {
            ...
            new_backrefs_array[new_back_ref_size] =
                      (BackwardReferenceFromDecoder){new_position, new_copy_len,
                                                new_distance, new_max_distance};
            new_back_ref_size++;
          }
        }
  ...
  *new_backward_references = new_backrefs_array;
...
void SaveNewBlock(size_t pos_begin, size_t pos_end, uint8_t old_type,
                  int* types_mapping, size_t old_num_types,
                  size_t old_num_blocks, BlockSplitFromDecoder* new_block_splits,
                  size_t* new_num_blocks, size_t* new_num_types) {
  assert(old_type < old_num_types);
  assert(*new_num_blocks < old_num_blocks);
  ...
  /* Create a new block */
  if (*new_num_blocks == 0 ||
      (new_block_splits->types[*new_num_blocks - 1] != types_mapping[old_type] &&
      pos_end - pos_begin > MIN_BLOCK_SIZE)) {
...
/* Adjusts old block splits for the case when [start, end) range is removed
    ...
    if (pos_begin < start) {
      if (pos_end <= start) {
        ...
        SaveNewBlock(pos_begin, pos_end, block_splits->types[i], types_mapping,
                     block_splits->num_types, block_splits->num_blocks,
        ...
        SaveNewBlock(pos_begin, pos_end - (end - start), block_splits->types[i],
                     types_mapping, block_splits->num_types, block_splits->num_blocks,
                     new_block_splits, &new_num_blocks, &new_num_types);
      }
      ...
      /* If current block starts after removing content (=> ends after)
         then save it*/
      SaveNewBlock(pos_begin - (end - start), pos_end - (end - start),
                   block_splits->types[i], types_mapping,
                   block_splits->num_types, block_splits->num_blocks,
                   new_block_splits, &new_num_blocks, &new_num_types);
    }
  }
...
/* Removes a [start, end) range from an uncompressed input_data
 * and saves it to output_data
 * Adjusts backward references and block splits for a new file
 */
...
/**
 * Compress a file which we get by removing a text segment from another,
 ...
 * encoded_buffer - an output compressed file which is
 *   an input uncompressed file with [start, end) range removed
 */
BROTLI_BOOL BrotliEncoderCompressSimilarDeletion(
    int quality, BrotliEncoderMode mode, size_t input_size,
    const uint8_t* input_buffer, int start, int end, size_t* encoded_size,
    uint8_t* encoded_buffer) {
  // TODO: should be fixed
  size_t decompressed_buffer_size = input_size * MAX_RISE;
  ...
  if (BrotliDecoderDecompress(input_size, input_buffer,
                              &decompressed_buffer_size, decompressed_data,
                              BROTLI_TRUE, &backward_references, &back_refs_size,
                              &literals_block_splits,
                              &insert_copy_length_block_splits) != 1) {
    BROTLI_LOG("Failure in BrotliDecompress\n");
  }
  ...
  BROTLI_LOG("Deleted data part\n");
  /* Compress new file with use if collected information */
  int lgwin = MinWindowLargerThanFile(removed_data_size, DEFAULT_WINDOW);
  ...
  FreeBlockSplits(&new_insert_copy_length_block_splits);
  return result;
}

#endif  /* BROTLI_COMPRESS_SIMILAR_H_ */
